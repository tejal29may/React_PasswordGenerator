  ...
  return (
    <>
    {
      ...
      <div className="Main_Container">
        <h1 style={{fontFamily:"cursive", color:"purple"}}>Password Generator</h1>
        <div className="output">
          <input type="text"  value={password} disabled />
          
          <button className="copy" onClick={copyToClipboard}><FaCopy/></button>
        </div>
        <div className="passselect">
          <label htmlFor="">
            <span style={{fontFamily:"cursive"}}>Select Pasword Length &nbsp;</span><br></br>(**8 to 50 Characters**){" "}
          </label>
          <input type="number" min={8} max={50} value={passlength} onChange={(e) => setpasslength(e.target.value)}/>
          {
            passlength>50?alert("Ayyooo password length is too much 😅") :""
          }
        </div>
        ...
        <button className="pass" onClick={generatePassword}>Generate Password</button>
      </div>
    </>
  );
}
